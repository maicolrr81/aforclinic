...
public class NoticeController {
    ...
    @GetMapping
    public ResponseEntity<?> getNoticePostList(PostPageRequest request, ApiPageRequest paging) {

        // 게시글 타입 설정
        request.setType(PostType.NOTICE);
    ...
    @GetMapping("/{id}")
    public ResponseEntity<?> getNoticePost(@PathVariable Long id) {
        return ApiResponse.ok(service.getPostById(id).orElseThrow(() -> new NotFoundException("게시글을 찾을 수 없습니다.")));
    }

    @PostMapping
    public ResponseEntity<?> createNoticePost(@RequestBody @Valid PostCreateRequest request) {
        service.createPost(request);
        return ApiResponse.ok();
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> updateNoticePost(@PathVariable Long id, @RequestBody @Valid PostUpdateRequest request) {
        request.setId(id);
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<?> partialUpdateNoticePost(@PathVariable Long id, @RequestBody PostUpdateRequest request) {
        request.setId(id);
        service.updatePost(request);
        return ApiResponse.ok();
    }

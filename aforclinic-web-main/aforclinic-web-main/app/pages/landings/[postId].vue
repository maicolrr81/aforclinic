...
const { data, error } = await useAsyncData<Partial<LandingPost>>(async () => {
  const data = await $api<ApiResponse<LandingPost>>(`/public/landings/${route.params.postId}`);
  return data.data;
});
...
<template>
  <article class="pb-[388px]">
    ...
    />
    <form
      class="fixed bottom-0 w-full max-w-screen-md bg-white p-2"
      @submit.prevent="submit(`REQUEST`)"
    >
      <div class="mx-auto max-w-md space-y-4">
        <p class=" bg-gray-100 py-4 text-center font-medium">
          본원에서는 고객님의 소중한 개인정보를<br>상담 외 어떠한 목적으로도 사용하지 않습니다.
        </p>
        <fieldset class="flex flex-col gap-4">
          <label class="grid w-full grid-cols-[60px_1fr] grid-rows-[1fr_auto]">
            <span class="bg-gray-300 py-4 text-center">이 름</span>
            <input
              v-model="nickname"
              maxlength="6"
          ...
          <label class="grid w-full grid-cols-[60px_1fr] grid-rows-[1fr_auto]">
            <span class="bg-gray-300 py-4 text-center">연락처</span>
            <input
              v-model="contact"
              class="flex-1 bg-gray-100 p-4"
              :class="{
        ...
        <div class="grid">
          <button class="bg-blue-500 py-4 text-xl font-bold text-white shadow-md hover:bg-blue-600">
            상담 신청하기
          </button>
        </div>
        <div class="!mt-4 grid grid-cols-3 gap-4">
          <button
            class="bg-blue-500 py-4 font-bold text-white shadow-md hover:bg-blue-600"
            type="button"
            @click="submit(`CALL`)"
          >
            전화문의

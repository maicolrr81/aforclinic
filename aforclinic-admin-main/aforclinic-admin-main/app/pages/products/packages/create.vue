...
import type { SelectedProductBundle } from '~~/shared/types/Product';
...
async function submit() {
  try {
    ...
    selectedBundleProducts.value.forEach((v) => {
      if (!categoryIdSet.has(v.categoryId)) {
        categoryList.push({ id: v.categoryId });
        categoryIdSet.add(v.categoryId);
      }
    ...
    $dialog.error(error, `패키지 등록 중 오류가 발생했습니다.`);
  }
}
</script>

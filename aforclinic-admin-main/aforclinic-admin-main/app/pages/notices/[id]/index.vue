...
async function remove() {
  const confirmed = await $dialog.confirm(`공지사항을 삭제하시겠습니까?`, {
    title: `⚠️ 주의`,
  });
  if (confirmed) {
    try {
      await $api(`/notices/${route.params.id}`, {
        method: `patch`,
        body: {
          status: `DELETED`,
        },
      });
      navigateTo(`/notices`, { replace: true });
    }
    catch {
      $dialog.alert(`공지사항 삭제 중 오류가 발생했습니다.`);
    }
  }
}
</script>
    ...
    <QCard>
      ...
      <QCardActions class="justify-between border-t p-4">
        ...
        <QBtn
          ...
          color="blue"
          label="수정"
          :to="`/notices/${route.params.id}/edit`"
        />
      </QCardActions>
    </QCard>
  </QPage>

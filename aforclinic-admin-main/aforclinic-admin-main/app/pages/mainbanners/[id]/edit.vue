...
const { data, error } = await useAsyncData<Partial<MainBanner>>(async () => {
  const data = await $api<ApiResponse<MainBanner>>(`/mainbanners/${route.params.id}`);
  return data.data;
}, {
  default: () => ({}),
});
...
async function remove() {
  const confirmed = await $dialog.confirm(`메인화면 배너를 삭제하시겠습니까?`, {
    title: `⚠️ 주의`,
  });
  ...
  try {
    await $api(`/mainbanners/${id.value}`, {
      method: `patch`,
    });
    navigateTo(`/mainbanners`, { replace: true });
  }
  catch (error) {
    $dialog.error(error, `메인화면 배너 삭제 중 오류가 발생했습니다.`);
  }
}
